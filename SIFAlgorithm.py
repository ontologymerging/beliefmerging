...
#ListOfInterpretations 
def SIFAlgorithm(ListofInterpretation):
	ListC = {}
	SIFT=[]
	for eachlist in ListofInterpretation:
		for each in eachlist:
			N = LIST_TAXONOMY_INTERSECTION[each]
			ListC[N] = []
			ListC[N].append(getIndividuals(each))

	for nameConcept, valueList in ListC:
		f = Frequency(valueList)
		t = len(nameConcept)/2
		for i,v in f:
			if v>t or v == len(ListC):
				SIFT.append(i)
	return SIFT
